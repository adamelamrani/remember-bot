...
class ChatsController implements ChatsControllerInterface {
  ...
  getChats = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      console.log(
        statusSelector(res.statusCode)('GET resquest to endpoint "/chat"'),
      );
  ...
  getChatById = async (req: Request, res: Response): Promise<void> => {
    ...
    if (
      ...
      res
        .status(400)
        .send(JSON.stringify({ message: 'Chat ID must be provided' }));
      return;
    }
  ...
  addChat = async (req: Request, res: Response): Promise<void> => {
    ...
    if (chat === undefined || chat === null) {
      res
        .status(400)
        .send(JSON.stringify({ message: 'Chat group must be provided' }));
      return;
    }
    ...
    if (chat.chatid === undefined || chat.chatid === null) {
      res
        .status(400)
        .send(JSON.stringify({ message: 'Chat ID cannot be empty' }));
      return;
    }
    ...
    if (chat.chatname === undefined || chat.chatname === null) {
      res
        .status(400)
        .send(JSON.stringify({ message: 'Chat name cannot be empty' }));
      return;
    }
    ...
    try {
      ...
      console.log(
        statusSelector(res.statusCode)(' POST resquest to endpoint "/chat"'),
      );
